...
import argparse
from MFCC.classify import load_gmm_model
from MFCC.common import extract_mfcc
...
def map_adaptation(gmm: GaussianMixture, mfcc_features: np.ndarray) -> GaussianMixture:
    ...
    if mfcc_features.shape[1] != np.asarray(gmm.means_).shape[1]:
    ...
    adapted_means = np.dot(posteriors.T, mfcc_features) / (posteriors.sum(axis=0)[:, None] + ulp)
    adapted_covariances = []
    
    for k in range(gmm.n_components):
        diff = mfcc_features - gmm.means_[k]
        weighted_diff = diff * posteriors[:, [k]]
        cov = np.dot(weighted_diff.T, diff) / (posteriors[:, [k]].sum() + ulp)
